    ...
    <div>
        <a-form-model :loading="loading" :model="formData" ref="dataForm" :label-col="dialogFormLabelCol" :wrapper-col="dialogFormWrapperCol" :rules="formRules">
            <a-form-model-item label="父级" prop="parent_id">
                <admin-permission-tree ref="adminPermissionTree" :default-data="parentId" @change="departmentChange"></admin-permission-tree>
            </a-form-model-item>
            ...
            <a-form-model-item label="路由" prop="route">
                <a-input v-model="formData.route" />
            </a-form-model-item>
...
module.exports = {
    ...
    components: {
        "admin-permission-tree":  httpVueLoader('/statics/components/adminPermission/adminPermissionTree.vue'),
    },
    props: {
        id: {
            default:function(){
    ...
    methods: {
        initForm(){
            ...
            if(this.$refs['adminPermissionTree']){
                this.$refs['adminPermissionTree'].clearData();
            }
        },
        submitData(){
            let that = this;
                ...
                if (valid) {
                    if(that.id){
                        ...
                        axios({
                            // 默认请求方式为get
                            method: 'post',
                            url: '/api/adminPermission/update',
                            // 传递参数
                            data: that.formData,
                            responseType: 'json',
                            headers:{
        ...
        getDetail(id){
            ...
            axios({
                // 默认请求方式为get
                method: 'post',
                url: '/api/adminPermission/getInfo',
                // 传递参数
                data: {
                    id:id
                },
                ...
                this.formData = {
                    ...
                    sort:res.data.adpe_sort,
                }
                if(res.data.adpe_parent_id){
                    this.parentId = res.data.adpe_parent_id;
                }
